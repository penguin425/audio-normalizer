...
//!
//! The repairer deliberately operates on container bytes instead of decoding
//! and re-encoding audio.  Unknown WAVE chunks, ADM XML, and essence bytes are
//! copied verbatim.  MXF is currently validate-and-copy only: rewriting KLV
//! metadata without a partition/index-table writer would be unsafe, so the
//! request fails closed when a mutation is requested for MXF.
...
pub fn evaluate(
    ...
    if has_mutation && source_format != "wave" {
        return Err(format!(
            "{} metadata mutation is unsupported; only WAVE/BWF byte-preserving repair is enabled (MXF and other containers are validate-and-copy only)",
            source_format
        ));
    }
...
fn validate_spec(spec: &MetadataRepairSpec) -> Result<(), String> {
