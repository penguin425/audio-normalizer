...
pub fn run_v2(options: &AdapterOptions) -> Result<MpeghAdapterReportV2, String> {
    ...
    let tool = crate::subprocess::run(process).map_err(|error| match error {
        ...
        crate::subprocess::Error::Spawn(error) => {
            format!("start MPEG-H adapter {}: {error}", adapter.display())
        }
        crate::subprocess::Error::Wait(error) => {
            format!("wait for MPEG-H adapter: {error}")
        }
        crate::subprocess::Error::Reader { stream, message } => {
            format!("read adapter {stream}: {message}")
        }
        error => format!("run MPEG-H adapter: {error}"),
    })?;
    if !tool.success() {
        return Err(format!(
    ...
    for presentation in response.presentations {
        ...
        let (rendered_sha256, rendered_bytes) =
            sha256_file_limited(&rendered, max_render_bytes, "MPEG-H adapter render")?;
...
pub fn inspect_mhas(path: &Path) -> Result<MhasInventory, String> {
    let (mut file, file_bytes) = open_regular_file(path, None).map_err(|error| match error {
        SafeFileError::Open(error) => format!("open MHAS stream {}: {error}", path.display()),
        SafeFileError::Metadata(error) => format!("stat MHAS stream {}: {error}", path.display()),
        SafeFileError::NotRegular | SafeFileError::Reparse | SafeFileError::TooLarge(_) => {
            format!("MHAS stream {} must be a regular file", path.display())
        }
    })?;
    if file_bytes == 0 {
...
fn render_byte_limit(max_decoded_samples: u64) -> Result<u64, String> {
    max_decoded_samples
        ...
        .ok_or_else(|| "MPEG-H render byte limit overflow".to_string())
}
...
fn workspace_byte_limit(
    ...
    let max_presentations = u64::try_from(max_presentations)
        .map_err(|_| "MPEG-H adapter presentation count does not fit in u64".to_string())?;
    let request_bytes = u64::try_from(request_bytes)
        .map_err(|_| "MPEG-H adapter request length does not fit in u64".to_string())?;
    max_render_bytes
        ...
        .ok_or_else(|| "MPEG-H adapter workspace byte limit overflow".to_string())
}
...
fn read_response(work: &Path, path: &Path) -> Result<Vec<u8>, String> {
    ...
    read_bounded_file(path, MAX_RESPONSE_BYTES, "MPEG-H adapter response")
}
...
mod tests {
    ...
    fn bounded_artifact_reads_reject_final_symlinks() {
        ...
        let error = read_bounded_file(&alias, 1024, "MPEG-H artifact").unwrap_err();
        assert!(error.contains("MPEG-H artifact"), "{error}");
        let error = sha256_file_limited(&alias, 1024, "MPEG-H artifact").unwrap_err();
    ...
    fn bounded_artifact_reads_enforce_size_limit() {
        ...
        assert!(read_bounded_file(&path, 4, "MPEG-H artifact").is_err());
        assert!(sha256_file_limited(&path, 4, "MPEG-H artifact").is_err());
    }
