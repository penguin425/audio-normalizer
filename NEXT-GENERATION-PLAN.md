...
| 5 | Stateless REST/gRPC service mode with job/request schemas, concurrency and decoded-sample limits, cancellation, and authentication hooks | Stage 4; **REST upload/analyze reference service shipped in v0.116.0** with bounded HTTP parsing, loopback-by-default binding, bearer-token hooks, and schema-validated responses. Optional gRPC `Analyze`/`Cancel`/`Health` RPCs and cooperative cancellation are implemented behind `grpc-service`; metrics remain follow-up work. |
