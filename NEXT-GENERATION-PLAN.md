...
| 6 | Prometheus text exporter and OpenTelemetry span/attribute bridge | Stage 5; **bounded, opt-in Prometheus and JSONL OpenTelemetry-compatible bridge implemented for the v0.118.0 release**. REST `GET /metrics` and gRPC `Metrics` expose fixed counters/histograms with no user-controlled labels; `--otel-jsonl` writes bounded server-span attributes for an external OTLP adapter. |
