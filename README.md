...
* **Exact adaptive true-peak pruning** uses a conservative FIR coefficient L1
  ...
  complete interpolation window is safe. Once active, one SIMD maximum
  reduction can prove a complete decoder chunk safe, advance the FIR history
  directly to its final 16 samples, and independently skip either stereo
...
* **Coalesced compressed-audio packets** append small planar-f32 Symphonia
  packets into reusable 4,096-frame chunks during the normalization render
  pass. This amortizes callback and writer overhead without splitting large
  packets or changing sample order. Analysis retains codec packet boundaries
  so True Peak pruning stays fine-grained on every architecture.
